    ...
    async def executeQueueCmd(self):
        ...
        while len(self.cmdQueue) > 0:
            ...
            if self.subscriptionDict[nodeID] is not None:
                ...
                task = asyncio.create_task(datacenterNode.write_value(val))
                # Add task to the set. This creates a strong reference to prevent the task being
                # garbage collected before it can finish.
                self.taskSet.add(task)
                # To prevent keeping references to finished tasks forever,
                # make each task remove its own reference from the set after
                # completion:
                task.add_done_callback(self.taskSet.discard)
...
async def main():
    ...
    def functionWrapper(*input_args):
        ...
        async def caller(parent, *inputArg):
            ...
            try:
                res = await client.nodes.objects.call_method(methodNode, *inputArg)
            except asyncio.TimeoutError:
                res = f"Timeout calling method {methodID}"
                print(res)
            return res
        ...
        while True:
            ...
            try:
                ...
                for node in listOfNode:
                    if "ns=2" in node.nodeid.to_string():
                        listOfMainNode.append(node)
                ...
                clientSubscription = await client.create_subscription(20, clientHandler)
                ...
                while True:
                    ...
                    if checkConectionPeriod == 1000:
                        taskList.append(loop.create_task(client.check_connection()))
                        checkConectionPeriod = 0
                    taskList.append(loop.create_task(clientHandler.executeQueueCmd()))
