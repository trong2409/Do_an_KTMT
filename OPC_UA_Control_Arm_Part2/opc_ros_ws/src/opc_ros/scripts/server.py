...
async def main():
    ...
    midServoNode = servo_folder.add_variable(idx, "mid", 90)
    leftServoNode = servo_folder.add_variable(idx, "left", 90)
    rightServoNode = servo_folder.add_variable(idx, "right", 90)

    add_on_folder = rootFolder.add_folder(idx, "Add-on")
    rotatorServoNode = add_on_folder.add_variable(idx, "rotator", 90)
    gripperServoNode = add_on_folder.add_variable(idx, "gripper", 90)
    ...
    imageJetmaxNode = imageFolder.add_variable(idx, "imageJetmax", b"a", datatype=ua.ObjectIds.ImageJPG)
    imageUSBCamNode = imageFolder.add_variable(idx, "imageUSBCam", b"a", datatype=ua.ObjectIds.ImageJPG)
    ...
    AIfolderNode = rootFolder.add_folder(idx, "AICurrent")
    
    AIEnterCurrentNode = AIfolderNode.add_variable(idx, "AIEnterCurrent", "", 
                                               datatype=ua.NodeId(ua.ObjectIds.String))
    
    AIRunCurrentNode = AIfolderNode.add_variable(idx, "AIRunCurrent", "",
                                             datatype = ua.NodeId(ua.ObjectIds.String))
    
    AIColorCurrentNode = AIfolderNode.add_variable(idx, "AIColorCurrent", "",
                                                   datatype= ua.NodeId(ua.ObjectIds.String))
    
    AIWasteTypeCurrentNode = AIfolderNode.add_variable(idx, "AIWasteType", "",
                                                       datatype = ua.NodeId(ua.ObjectIds.String))
    
    AIWasteStateCurrentNode = AIfolderNode.add_variable(idx, "AIWasteState", 1,
...
# -----------------------------------------------------------------------------------------------------------------------------------#
    ...
    async def updateVideoJetMax():
        while not rospy.is_shutdown():
            if (len(imageQueue) > 0):
                ...
                imageJetmaxNode.set_value(compressed_image.getvalue())
    ...
    async def updateVideoUSBCam():
        while not rospy.is_shutdown():
            if len(imageQueue2) > 0:
                ...
                # print(len(compressed_image.getvalue()), flush=True)
                imageUSBCamNode.set_value(compressed_image.getvalue())
            await asyncio.sleep(0.05)
    ...
    def updateAIEnter(data):
        AIEnterCurrentNode.set_value(data.data)
        
    def updateAIRun(data):
        AIRunCurrentNode.set_value(data.data)
            
    def updateAIColor(data):
        AIColorCurrentNode.set_value(data.data)
            
    def updateAIWasteType(data): 
        AIWasteTypeCurrentNode.set_value(data.data)
        
    def updateAIWasteState(data):
        AIWasteStateCurrentNode.set_value(data.data)
    ...
    async def ros_run():
        ...
        rospy.Subscriber('/jetmax/AIEnterCurrent', String, callback = updateAIEnter)
        rospy.Subscriber('/jetmax/AIRunCurrent', String, callback = updateAIRun)
        rospy.Subscriber('/jetmax/AIColorCurrent', String, callback = updateAIColor)
        rospy.Subscriber('/waste_classification/set_suck',UInt16, callback = updateAIWasteState)
        rospy.Subscriber('/waste_classification/set_waste_class',String, callback = updateAIWasteType)
