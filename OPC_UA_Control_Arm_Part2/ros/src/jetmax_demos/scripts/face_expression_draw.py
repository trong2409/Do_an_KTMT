...
def draw_exp(name):
    ngcs = {
        'Angry': 'gcode/angry.ngc',
        'Happy': 'gcode/happy.ngc',
        'Neutral': 'gcode/neutral.ngc',
        'Surprise': 'gcode/surprise.ngc',
    }
    try:
        if name in ngcs:
            simple_draw.draw_ngc(ngcs[name])
            jetmax.go_home(1)
            rospy.sleep(1.2)
    finally:
        state.runner = None
...
def image_proc_a(image):
    ...
    if state.face_tracking is None:
        ...
        if get_face:
            ...
            if state.count > 7:
                state.count = 0
                state.runner = threading.Thread(target=draw_exp, args=(a,))
                state.runner.start()
        else:
            state.count = 0
    return img_ret
