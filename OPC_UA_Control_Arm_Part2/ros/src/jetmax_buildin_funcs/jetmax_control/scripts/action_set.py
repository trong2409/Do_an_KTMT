...
class ActionSetRunner:
    ...
    def get_action_list_cb(self, req):
        for root, dirs, files in os.walk('/home/hiwonder/ActionSets'):
            ...
            print(dirs)
            print(files)
            files = list(map(lambda n: n[:-5], files))
            return ActionSetListResponse(action_sets=files)

    def save_actionset_cb(self, req):
        with open(os.path.join('/home/hiwonder/ActionSets', req.file_name + '.json'), 'w') as f:
            print(req.data)
            f.write(req.data)
        return [True, '']
    ...
    def read_actionset_cb(self, req):
        try:
            with open(os.path.join('/home/hiwonder/ActionSets', req.file_name + '.json')) as f:
                data = json.load(f)
    ...
    def remove_actionset_cb(self, req):
        os.remove(os.path.join('/home/hiwonder/ActionSets', req.file_name + '.json'))
        return [True, '']
